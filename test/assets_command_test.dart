...
''');
    ...
    test('generates AppAssets class with type-safe constant paths', () async {
      final assetsImages = Directory(p.join(tempDir.path, 'assets', 'images'))
        ..createSync(recursive: true);
      final assetsIcons = Directory(p.join(tempDir.path, 'assets', 'icons'))
        ..createSync(recursive: true);
      ...
      final generatedFile = File(
        p.join(tempDir.path, 'lib', 'constants', 'app_assets.dart'),
      );
      ...
      expect(
        content,
        contains("static const String logoPng = 'assets/images/logo.png';"),
      );
      expect(
        content,
        contains(
          "static const String homeIconSvg = 'assets/icons/home_icon.svg';",
        ),
      );
    });
    ...
    test('detects unused assets and deletes them when requested', () async {
      ...
      final usedFile = File(p.join(assetsImages.path, 'used.png'))
        ..writeAsStringSync('');
      final unusedFile = File(p.join(assetsImages.path, 'orphan.png'))
        ..writeAsStringSync('');

      // Reference only used.png in code
      File(
        p.join(libDir.path, 'main.dart'),
      ).writeAsStringSync("final img = 'assets/images/used.png';");
