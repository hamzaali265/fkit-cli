...
''');
    ...
    test(
      'configures Android Gradle Kotlin, iOS xcconfigs, entrypoints, and vscode launch',
      () async {
...
''');
        ...
        Directory(
          p.join(tempDir.path, 'ios', 'Flutter'),
        ).createSync(recursive: true);
        ...
        expect(
          File(
            p.join(tempDir.path, 'ios', 'Flutter', 'Debug-dev.xcconfig'),
          ).existsSync(),
          isTrue,
        );
        expect(
          File(
            p.join(tempDir.path, 'ios', 'Flutter', 'Release-prod.xcconfig'),
          ).existsSync(),
          isTrue,
        );
        ...
        expect(
          File(p.join(tempDir.path, 'lib', 'main_dev.dart')).existsSync(),
          isTrue,
        );
        expect(
          File(p.join(tempDir.path, 'lib', 'main_staging.dart')).existsSync(),
          isTrue,
        );
        expect(
          File(p.join(tempDir.path, 'lib', 'main_prod.dart')).existsSync(),
          isTrue,
        );
